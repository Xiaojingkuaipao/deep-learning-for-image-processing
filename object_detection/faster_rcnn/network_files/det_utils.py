...
class BalancedPositiveNegativeSampler(object):
    ...
    def __call__(self, matched_idxs):
        ...
        for matched_idxs_per_image in matched_idxs:
            ...
            pos_idx_per_image = positive[perm1] # 被采样为正样本的anchor的索引
            neg_idx_per_image = negative[perm2] # 被采样为负样本的anchor的索引

            # create binary mask from indices
            pos_idx_per_image_mask = torch.zeros_like(
                matched_idxs_per_image, dtype=torch.uint8
            ) # 创建一个和原来match形状一样的全0tensor
            neg_idx_per_image_mask = torch.zeros_like(
                matched_idxs_per_image, dtype=torch.uint8
            )
            # 在正样例的模板中只有正样例的地方置为1，在负样例的模板中只有负样例的地方为1
...
class BoxCoder(object):
    ...
    def decode_single(self, rel_codes, boxes):
        ...
        dx = rel_codes[:, 0::4] / wx   # 预测anchors/proposals的中心坐标x回归参数 使用这样的切片方式是为了维度一定，如果只使用[:,0]就只剩下一个维度了
...
class Matcher(object):
    ...
    def __call__(self, match_quality_matrix):
        ...
        # matches对应最大值所在的索引, 也就是这个anchor匹配到了哪个gt_box
        ...
        if self.allow_low_quality_matches: # 是否启用论文中提到的第一个规则，也就是有些情况下gt与anchor的iou都不大，有的gt没有匹配到anchor,需要为这种gt匹配anchor
    ...
    def set_low_quality_matches_(self, matches, all_matches, match_quality_matrix):
        ...
        # highest_quality_foreach_gt为匹配到的最大iou值, 每个gt与anchor的最大iou值
