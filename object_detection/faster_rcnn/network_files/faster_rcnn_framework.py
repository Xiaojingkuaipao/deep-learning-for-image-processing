...
class FasterRCNNBase(nn.Module):
    ...
    def forward(self, images, targets=None):
        ...
        # TorchScript中指定变量的类型和值，对于生成高效的代码十分有用
...
class FasterRCNN(FasterRCNNBase):
    ...
    def __init__(self, backbone, num_classes=None,
        ...
        rpn = RegionProposalNetwork(
            ...
            rpn_fg_iou_thresh, # RPN用于归类于前景的阈值
            rpn_bg_iou_thresh, # RPN用于归类于背景的阈值
            rpn_batch_size_per_image, # RPN计算损失的时候使用的正负样本的总个数
            rpn_positive_fraction, # 采样为正样本的比例
            rpn_pre_nms_top_n, # NMS之前每个特征层保留的目标个数
            rpn_post_nms_top_n, # NMS之后剩余的目标个数， 也就是RPN生成的Proposal的数目
            rpn_nms_thresh, # NMS处理时的阈值
            score_thresh=rpn_score_thresh)

        #  Multi-scale RoIAlign pooling
        if box_roi_pool is None:
            box_roi_pool = MultiScaleRoIAlign( # pytorch官方实现的roi pooling层，更加好
