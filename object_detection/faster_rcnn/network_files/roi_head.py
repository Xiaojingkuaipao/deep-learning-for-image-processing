...
def fastrcnn_loss(class_logits, box_regression, labels, regression_targets):
    ...
    sampled_pos_inds_subset = torch.where(torch.gt(labels, 0))[0] # 找出正样本(非背景类)
    ...
    box_loss = det_utils.smooth_l1_loss(
        # 获取指定索引proposal的指定类别box信息, 计算smoothl1损失的时候要指定正样本的位置以及其类别
...
class RoIHeads(torch.nn.Module):
    ...
    def __init__(self,
                 ...
                 fg_iou_thresh, bg_iou_thresh,  # default: 0.5, 0.5 确定正负样本的时候的iou阈值
                 batch_size_per_image, positive_fraction,  # default: 512, 0.25 每张图片上采样多少个proposal， 正样本所占比例
                 bbox_reg_weights,  # None
                 # Faster R-CNN inference
                 score_thresh,        # default: 0.05
                 nms_thresh,          # default: 0.5 nms处理时的iou阈值
        ...
        self.box_roi_pool = box_roi_pool    # Multi-scale RoIAlign pooling ROI Pooling层
        self.box_head = box_head            # TwoMLPHead 中间的全连接层
        self.box_predictor = box_predictor  # FastRCNNPredictor 框回归和预测层
    ...
    def assign_targets_to_proposals(self, proposals, gt_boxes, gt_labels):
        ...
        for proposals_in_image, gt_boxes_in_image, gt_labels_in_image in zip(proposals, gt_boxes, gt_labels):
            ...
            matched_idxs.append(clamped_matched_idxs_in_image) # proposal匹配到了哪个gt box
            labels.append(labels_in_image) # 每个proposal的的类别标签值
    ...
    def select_training_samples(self,
        ...
        # 为每个proposal匹配对应的gt_box，并划分到正负样本中， 返回的是proposal匹配到了哪个样本框以及对应的标签
        matched_idxs, labels = self.assign_targets_to_proposals(proposals, gt_boxes, gt_labels)
        # sample a fixed proportion of positive-negative proposals
        # 按给定数量和比例采样正负样本
        sampled_inds = self.subsample(labels) # list[Tensor]每个元素是图片中被采样的proposal的索引值
        matched_gt_boxes = []
