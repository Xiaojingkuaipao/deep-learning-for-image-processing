...
class AnchorsGenerator(nn.Module):
    ...
    def generate_anchors(self, scales, aspect_ratios, dtype=torch.float32, device=torch.device("cpu")):
        ...
        Arguments:
            ...
            device: cpu/gpu
        Returns:
            base_anchors: [len(scales) * len(aspect_ratios), 4] anchor的模板
        """
    ...
    def set_cell_anchors(self, dtype, device):
        ...
        self.cell_anchors = cell_anchors # cell anchors 里面存的是anchor模板，是一个list，list中的 元素是tensor，
    ...
    def grid_anchors(self, grid_sizes, strides):
        ...
        for size, stride, base_anchors in zip(grid_sizes, strides, cell_anchors):
            ...
            shifts_x = torch.arange(0, grid_width, dtype=torch.float32, device=device) * stride_width  # 生成网格的左上角坐标
            # shape: [grid_height] 对应原图上的y坐标(行)
            shifts_y = torch.arange(0, grid_height, dtype=torch.float32, device=device) * stride_height # 生成网格左上角坐标
    ...
    def forward(self, image_list, feature_maps):
        ...
        """
        Args:
            image_list: 经过transform之后的原始图像
            feature_maps: 经过backbone之后的特征图 list[Tensor], len(feature_maps) = 特征金字塔层数
        Return:
            anchor: list[Tensor]，len(anchors) = batch_size, Tensor.shape = [num_all_anchors_per_image, 4]
        """
...
class RPNHead(nn.Module):
    ...
    Arguments:
        ...
        num_anchors: number of anchors to be predicted， 每个位置生成的anchor个数, num_size * num_ratios
    """
...
def concat_box_prediction_layers(box_cls, box_regression):
    ...
    Returns:
        box_cls: [整个batch中所有图片的所有特征图的anchor总数,  1]
        box_regression: [整个batch中所有图片的所有特征图的anchor总数, 4]
    """
    box_cls_flattened = []
...
class RegionProposalNetwork(torch.nn.Module):
    ...
    Arguments:
        anchor_generator (AnchorGenerator): module that generates the anchors for a set of feature
            maps.
        head (nn.Module, The RPNHead): module that computes the objectness and regression deltas
    ...
    def __init__(self, anchor_generator, head,
        ...
        self.min_size = 1.  # 过滤proposal的时候会使用到的参数
    ...
    def assign_targets_to_anchors(self, anchors, targets):
        ...
        for anchors_per_image, targets_per_image in zip(anchors, targets):
            ...
            if gt_boxes.numel() == 0:
                ...
                matched_gt_boxes_per_image = gt_boxes[matched_idxs.clamp(min=0)] # 每个anchor匹配到的gt box的坐标

                # 记录所有anchors匹配后的标签(正样本处标记为1，负样本处标记为0，丢弃样本处标记为-2)
                labels_per_image = matched_idxs >= 0
                labels_per_image = labels_per_image.to(dtype=torch.float32) # 正样本anchor等于1，其余等于0

                # background (negative examples)
                bg_indices = matched_idxs == self.proposal_matcher.BELOW_LOW_THRESHOLD  # -1 负样本的anchor等于1，其余等于0
                labels_per_image[bg_indices] = 0.0 # 模板中anchor是负样本的位置置为0 因为有物体标签为1，没有物体标签为0

                # discard indices that are between thresholds
                inds_to_discard = matched_idxs == self.proposal_matcher.BETWEEN_THRESHOLDS  # -2
                labels_per_image[inds_to_discard] = -1.0 # 模板中anchor为丢弃的无用的样本的位置置为-1.0
    ...
    def compute_loss(self, objectness, pred_bbox_deltas, labels, regression_targets):
        ...
        Arguments:
            objectness (Tensor)：预测的前景概率 [all anchors of minibatch, 1]
            pred_bbox_deltas (Tensor)：预测的bbox regression [all anchors of minibatch, 4]
            labels (List[Tensor])：真实的标签 1, 0, -1（batch中每一张图片的labels对应List的一个元素中） List[Tensor] len=batch_size
            regression_targets (List[Tensor])：真实的bbox regression List[Tensor] len=batch_size
    ...
    def forward(self,
        ...
        # objectness和pred_bbox_deltas都是list，list中每个元素是对于当前特征层的预测,也就是说len()=特征层数
        # [bs, num_anchor, H, W] 和[bs, num_anchor * 4, H, W]
